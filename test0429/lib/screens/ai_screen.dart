...
class AiScreen extends StatelessWidget {
  ...
  static const String habitInsight = '저번주 이 시간에도 스타벅스에 5,500원 쓰셨는데\n혹시 습관이 된 건 아닌가요?';
  ...
  static const List<Map<String, dynamic>> nagHistory = [
    {'date': '04.02', 'location': '스타필드 코엑스', 'msg': '예산의 64%를 사용했어요. 오늘 예산 15,300원 남았습니다.'},
    {'date': '03.28', 'location': '신세계백화점 강남점', 'msg': '이번 달 쇼핑에 이미 98,000원 썼어요. 한 번만 참아봐요!'},
    {'date': '03.21', 'location': '홍대 거리', 'msg': '주말 지출 주의! 지난 주말에도 43,000원 썼었어요.'},
  ];
  // ────────────────────────────────────────────────────────
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(topCategoryInsight, style: AppTextStyles.heroAmount.copyWith(fontSize: 18, letterSpacing: 0)),
          const SizedBox(height: 16),
          Divider(color: Colors.white.withValues(alpha: 0.15)),
          const SizedBox(height: 12),
          _InsightRow(icon: Icons.show_chart, color: AppColors.stateDanger, text: changeInsight),
          const SizedBox(height: 10),
          _InsightRow(icon: Icons.psychology, color: AppColors.stateCaution, text: habitInsight),
        ],
      ),
    );
  }
  ...
  Widget _buildHorizontalInsightCards() {
    const items = [
      {'icon': Icons.calendar_today, 'title': '가장 많이 쓴 요일', 'value': '토요일', 'isWarning': false},
      {'icon': Icons.trending_up, 'title': '지난주 대비 지출', 'value': '+12,300원', 'isWarning': true},
      {'icon': Icons.emoji_events, 'title': '챌린지 달성률', 'value': '1 / 3개', 'isWarning': false},
    ];
    return SizedBox(
      height: 110,
      child: ListView(
        ...
        children: (items as List<Map<String, dynamic>>).map((item) {
          return Container(
            ...
            child: Column(
              ...
              children: [
                Icon(item['icon'] as IconData, color: AppColors.primary.withValues(alpha: 0.7), size: 22),
                const Spacer(),
                ...
                Text(
                  ...
                  style: AppTextStyles.bodyBold.copyWith(
                    color: (item['isWarning'] as bool) ? AppColors.stateDanger : AppColors.textPrimary,
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            child: Stack(
              ...
              children: [
                PieChart(PieChartData(
                  ...
                  sections: categoryStats.map((s) => PieChartSectionData(
                    color: s['color'],
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text('100%', style: AppTextStyles.bodyBold.copyWith(fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                  ],
                ),
              ],
            ),
          ...
          const SizedBox(height: 16),
          Column(children: categoryStats.map((s) => _buildCategoryRow(s)).toList()),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(stat['name'], style: AppTextStyles.body.copyWith(color: AppColors.textPrimary))),
            ...
            Row(
              children: [
                Icon(isUp ? Icons.arrow_drop_up : Icons.arrow_drop_down,
                    color: isUp ? AppColors.expense : AppColors.income, size: 18),
                Text(
                  '${change.abs()}%',
                  style: AppTextStyles.caption.copyWith(color: isUp ? AppColors.expense : AppColors.income),
                ),
              ],
            )
          else
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: AppColors.white,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: AppColors.stateDanger.withValues(alpha: 0.25)),
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.location_on, color: AppColors.stateDanger, size: 18),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(item['location'], style: AppTextStyles.bodyBold.copyWith(color: AppColors.textPrimary)),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.auto_awesome, color: AppColors.primaryLight, size: 13),
          const SizedBox(width: 4),
          Text('AI 생성 · 4월 요약',
              style: AppTextStyles.micro.copyWith(color: AppColors.primaryLight, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Expanded(child: Text(text, style: AppTextStyles.body.copyWith(color: Colors.white70))),
      ],
    );
  }
}
